...
- (void)setMediaModel:(JPMediaModel *)mediaModel
{
    ...
    if (_mediaModel.thumImage == nil) {
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            ...
            _mediaModel.thumImage = [JPVideoUtil getFirstImageWithVideoUrl:videoUrl];
            CGSize imageSize  = [JPVideoUtil getVideoSizeWithUrl:videoUrl];
            _mediaModel.videoFrameSize = [NSString stringWithFormat:@"%.0f*%.0f", imageSize.width, imageSize.height];
            _mediaModel.videoDurationStr = [NSString stringWithTimeInterval:_mediaModel.videoDuration];
            NSFileManager *fileManager = [NSFileManager defaultManager];
            NSInteger bytes = [[fileManager attributesOfItemAtPath:mediaModel.videoLocalPath error:nil] fileSize] / 1000;
            if (bytes < 1000) {
                _mediaModel.videoBytes = [NSString stringWithFormat:@"%ld KB", bytes];
            }else{
                _mediaModel.videoBytes = [NSString stringWithFormat:@"%ld MB", bytes / 1000];
            }
