...
- (IBAction)changeTimeTypeAction:(id)sender {
    ...
    if (_videoModel.timePlayType == JPVideoTimePlayTypeNone) {
        [_timeTypeButton setImage:[UIImage imageNamed:@"time1.0"] forState:UIControlStateNormal];
    }else if (_videoModel.timePlayType == JPVideoTimePlayTypeFast)
    {
        [_timeTypeButton setImage:[UIImage imageNamed:@"time5.0"] forState:UIControlStateNormal];
    }else{
        [_timeTypeButton setImage:[UIImage imageNamed:@"time0.2"] forState:UIControlStateNormal];
    }
...
- (void)setVideoModel:(JPVideoModel *)videoModel
{
    ...
    canFast = YES;
    canClid = YES;
    if (CMTimeCompare(videoModel.timeRange.duration, CMTimeMake(4, 1)) < 0) {
        canFast = NO;
    }
